...
class GreedyHopRouting(Algorithm):
        ...
        while any(flag_box):
            ...
            for src,dst in cycle(self.srcDstPairs):
                ...
                while (True) :
                    ...
                    if (last == dst):
                        break
                    next = sorted(list(filter(lambda neighbor : (neighbor.remainingQubits > 2 or neighbor == dst and neighbor.remainingQubits > 1) and
                        len(list(filter( lambda x: (not x.assigned) and x.contains(neighbor),last.links))) > 0   ,
                                                                    last.neighbors)), lambda x: self.topo.hopsAway[Edge(x,dst)])[0]
    ...
    def P4(self):
        for _,width,p in self.pathsSortedDynamically:
            ...
            for n12,next in tmp:
                prev,n = n12
                prevLinks =  list(filter( lambda x: x.entangled and ((x.n1==prev) and not x.s2 or (x.n2 ==prev and not x.s1)) ,n.links))[0:width]
                nextLinks =  list(filter( lambda x: x.entangled and ((x.n1==next) and not x.s2 or (x.n2 ==next and not x.s1)) ,n.links))[0:width]
