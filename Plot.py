from QN_routing.configs import referenceSetting, nsdList, topoRange, parseLog, id
...
class Plot:
    ...
    def rp2Cdf_nsd(self):
        ...
        def getY1(result1, max1):
            return list(map(lambda l: l+[1 for _ in range(max1+1-len(l))] if len(l) < max1+1 else l, result1))
        ...
        def getY2(result2, max2):
            return list(map(lambda l: l+[1 for _ in range(max2+1-len(l))] if len(l) < max2+1 else l, result2))
        ...
        string2 = temp1.substitute(getName2=getName2(d, n, p, q, k), getSolutionList2=getSolutionList2(nsdList),
